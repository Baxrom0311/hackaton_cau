...
def main():
    ...
    train_loader = DataLoader(SegmentationDataset(train_img_dir, train_mask_dir, get_train_transforms(CFG.IMG_SIZE)), 
                              batch_size=CFG.BATCH_SIZE, shuffle=True, num_workers=CFG.NUM_WORKERS, pin_memory=True, drop_last=True)
    val_loader = DataLoader(SegmentationDataset(val_img_dir, val_mask_dir, get_val_transforms(CFG.IMG_SIZE)), 
    ...
    print(f"\n{'='*60}\n  ✅ Training Done! Best IoU: {best_iou:.4f} | Threshold: {best_th:.2f} | Total ⏱️ {(time.time()-start_time)/60:.1f} min\n{'='*60}")
