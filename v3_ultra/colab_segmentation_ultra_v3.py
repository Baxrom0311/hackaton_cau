# ============================================================
# AI Healthcare Hackathon 2026 — ULTRA Segmentation (Colab T4/A100)
# ============================================================
# 🚨 MAQSAD: Segmentation bo'yicha maksimal 40 balni olish (Mean IoU > 0.85)
# 
# ✅ Encoder: timm-efficientnet-b5 (SOTA tibbiyot uchun)
# ✅ Decoder: UnetPlusPlus
# ✅ Resolution: 512x512 (Kichik detallarni asayuvchi max resolution)
# ✅ Augmentations: ElasticTransform, CLAHE, GridDropout
# ✅ Optimizer/Scheduler: CosineAnnealingWarmRestarts (T_0=15)
# ✅ Loss: Lovász Hinge + DiceBCE
# ✅ Ensemble: SWA + TTA (4x) + Morphological Post-processing
# ✅ Gradient Accumulation (T4 xotira yetishmovchiligini oldini olish uchun)
# ============================================================

# Cell 1: install
# !pip install segmentation-models-pytorch albumentations timm -q
...
class CFG:
    ...
    MODEL_SAVE_DIR = "/content/drive/MyDrive/hackaton_models/segmentation_ultra"

    # ULTRA: timm-efficientnet-b5
    ...
    # Training
    EPOCHS = 50
    BATCH_SIZE = 8                    # 512x512 b5 uchun 8-16 qilinadi
    GRAD_ACCUM_STEPS = 2              # Effective batch size = 16 bo'ladi
    LR = 3e-4
    WEIGHT_DECAY = 1e-4
    NUM_WORKERS = 2
    IMG_SIZE = 512                    # Max resolution
    SEED = 42
    VAL_EVERY = 2
...
def get_train_transforms(img_size):
    return A.Compose([
        ...
        # Ultra Medical Transforms
        A.ElasticTransform(alpha=120, sigma=120 * 0.05, alpha_affine=120 * 0.03, p=0.4),
        A.CLAHE(clip_limit=3.0, p=0.3),  # Contrast enhancement
        A.GridDistortion(p=0.3),
        A.Affine(translate_percent=(-0.1, 0.1), scale=(0.8, 1.2), rotate=(-30, 30), p=0.4),
        A.OneOf([
    ...
    for i, (images, masks) in enumerate(pbar):
        ...
        if (i + 1) % CFG.GRAD_ACCUM_STEPS == 0 or (i + 1) == len(loader):
            ...
            if scheduler is not None and not isinstance(scheduler, torch.optim.swa_utils.SWALR):
                scheduler.step(epoch + i / len(loader)) # Step WarmRestarts per batch
...
# ─── Main ────────────────────────────────────────────────────────────────────
def main():
    global epoch # to make it accessible to scheduler step inside train_one_epoch
    seed_everything(CFG.SEED)
    os.makedirs(CFG.MODEL_SAVE_DIR, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"  🚀 ULTRA Segmentation Training (Mean IoU Maximizer)")
    print(f"  Device : {CFG.DEVICE} | {torch.cuda.get_device_name(0) if CFG.DEVICE == 'cuda' else ''}")
    print(f"  Encoder: {CFG.ENCODER} | ImgSize: {CFG.IMG_SIZE}")
    print(f"  🔥 SOTA Augments + SWA + TTA + WarmRestarts + GradAccum")
    print(f"{'='*60}\n")

    train_dataset = SegmentationDataset(CFG.TRAIN_IMG_DIR, CFG.TRAIN_MASK_DIR, transform=get_train_transforms(CFG.IMG_SIZE))
    val_dataset = SegmentationDataset(CFG.VAL_IMG_DIR, CFG.VAL_MASK_DIR, transform=get_val_transforms(CFG.IMG_SIZE))
